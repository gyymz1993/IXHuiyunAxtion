 ...
 *
 */
public class TCPJsonServerConnect {
	...
	public TCPJsonServerConnect(int port) {
		try {
			...
			new Thread(){
				public void run() {
					System.out.println("创建TCPJsonServerConnect线程，线程名为" + currentThread().getName());
					...
					while(!server.isClosed()){
						...
						try {
							...
							// 每个socket连接都有自己的线程
							TCPJsonConnect connect = new TCPJsonConnect(socket,listSocket);
							connect.setOnReceiveCmdListener(new OnReceiveCmdListener() {
							...
							System.out.println("TCPJsonServerConnect的IOException");
						}
					}
					
					System.out.println("关闭TCPJsonServerConnect线程，线程名为" + currentThread().getName());
				};
			}.start();
						...
						if(NetworkSupport.getSocketId(socket) == socketId){
							try {
								socket.getOutputStream().write(
										JsonPack.Data_Pack(Data.CUSTOM_CODE_RF, cmd));
							} catch (IOException e) {
								e.printStackTrace();
							}
						}
