...
/**   
 * @Description: 下载从机信息 
 * @date 2015-2-27 下午1:47:55 
 * @version V1.0   
 */
public class DownloadSlave_57 extends BaseAnalyst {
	private final int DOWNLOAD_SLAVE = 57;
	private final int RE_DOWNLOAD_SLAVE = 58;
	...
	public void handleData(int socketId, BaseJsonObj jsonObj) {
		...
		try {
			if (OBJ_MASTER.equals(jsonObj.getObj())
					&& DOWNLOAD_SLAVE == jsonObj.getCode()) {
				downloadSlave(socketId, jsonObj);
			}
		} catch (Exception e) {
			e.printStackTrace();
	 ...
	 * 
	 * @author lzn 2015年1月10日 上午9:53:18
	 * @param jsonObj
	 * @param socketId
	 */
	private void downloadSlave(int socketId, BaseJsonObj jsonObj) {
		if (jsonObj instanceof DataJsonObj) {
			...
			send_jsonObj.setCode(RE_DOWNLOAD_SLAVE);
			...
			try {
				if (TokenManager.judgetToken(resultMap.get("token").toString()) == null) {
					...
					List<Slave> allSlave = deviceDao.getAllSlave();
					
					if (allSlave != null && allSlave.size() > 0) {
						for (Slave s : allSlave) {
							Map<String, String> maps = new HashMap<String, String>();
							maps.put("address", s.getAddr() + "");
							maps.put("type", s.getType() + "");
							list.add(maps);
						}
						sendData.list=list;
